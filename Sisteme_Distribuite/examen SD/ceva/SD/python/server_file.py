...
class Price:
    def __init__(self, token, symbol):
        self.token = token
        self.symbol = symbol
        self.url = 'https://finnhub.io/api/v1/stock/price-target?symbol='+symbol+'&token='+token

    def get_price(self):
        print("caut resursa")
        f = open("/home/ana/Desktop/SD/python/file.txt")
        content = f.read()
        print("am obtinut\n")
        print(content)
        lista = content.split('}')
...
class Server:
    ...
    def get_list(self):
        print("Fac cerere pentru resursa!")
        self.prices = self.p.get_price()
