...
class AgendaController {
    ...
    @Autowired
    private lateinit var cacheInterface: CacheInterface
    ...
    fun search(@RequestParam(required = false, name = "lastName", defaultValue = "") lastName: String,
        ...
        var l = cacheInterface.search(lastName)
        var f = cacheInterface.search(firstName)

        var personList:List<Person>

        if(!l.isEmpty())
        {
            //inf se gasesc in cache
            personList = l
        }
        else
        {
            ...
            else
            {
                ...
                cacheInterface.write(personList)
            }
        }
