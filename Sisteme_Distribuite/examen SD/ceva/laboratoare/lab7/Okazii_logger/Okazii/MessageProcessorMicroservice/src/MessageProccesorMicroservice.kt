...
class MessageProcessorMicroservice {
    ...
    init {
        ...
        loggerSocket.getOutputStream().write(Message.create("MessageProcessor: ", "MessageProcessorMicroservice se executa pe portul: ${messageProcessorSocket.localPort}").serialize())
        loggerSocket.getOutputStream().write(Message.create("MessageProcessor: ", "Se asteapta mesaje pentru procesare...").serialize())
        ...
        val receiveInQueueSubscription = receiveInQueueObservable
            ...
            .subscribeBy(
                onNext = {
                    ...
                    loggerSocket.getOutputStream().write(Message.create("MessageProcessor: ", "Am primit ${message}. Adaug in coada").serialize())
                    messageQueue.removeAll(messageQueue.filter { messageQueue.count{ it == message } > 1 })
                ...
                onComplete = {
                    ...
                    loggerSocket.getOutputStream().write(Message.create("MessageProcessor: ", finishedMessagesMessage.toString()).serialize())
                ...
                onError = {
                    loggerSocket.getOutputStream().write(Message.create("MessageProcessor: ", "Eroare $it").serialize())
                    println("Eroare: ${it}") }
            )
    ...
    private fun sendProcessedMessages() {
        try {
            ...
            loggerSocket.getOutputStream().write(Message.create("MessageProcessor: ", "Trimit urmatoarele mesajele:").serialize())
            Observable.fromIterable(messageQueue).subscribeBy(
                onNext = {
                    println(it.toString())
                    loggerSocket.getOutputStream().write(Message.create("MessageProcessor: ", it.toString()).serialize())
                ...
                onComplete = {
                    ...
                    loggerSocket.getOutputStream().write(Message.create("MessageProcessor: ", noMoreMessages.toString()).serialize())
