...
class BidderMicroservice(private var firstName:String,
    ...
    private fun bid() {
        ...
        val biddingMessage =  Message.create("${auctioneerSocket.localAddress}:${auctioneerSocket.localPort}",
            "Eu ${firstName}  ${lastName} ${email} ${phone} licitez ${pret}")
...
fun main(args: Array<String>) {
    ...
    if(args.size < 4)
    {
        println("Linia de comanda este gresita!\n" +
                "Trebuie sa introduceti numele, prenumele, telefonul si email")
        return
    }
    var firstName:String = args[0]
    var lastName :String = args[1]
    var phone :String = args[2]
    var email :String = args[3]
    print(firstName +"\n"+lastName+"\n"+ phone+"\n"+email)
    //print(lastName)
    val bidderMicroservice = BidderMicroservice( firstName, lastName,
                                    phone, email)
