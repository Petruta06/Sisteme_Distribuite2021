...
import org.examen.SQLiteExample
...
class BeerDAOService : BeerDAO{
    ...
    override fun createBeerTable() {
        jdbcTemplate.execute(
            """CREATE TABLE IF NOT EXISTS beers(id INTEGER PRIMARY KEY AUTOINCREMENT, name VARCHAR(100) UNIQUE, price FLOAT)"""
        )
    }

    override fun addBear(beer: Beer) {
        if(pattern.matcher(beer.beerName).find()){
            println("SQL Injection for beer name")
            return
        }
        jdbcTemplate.update(
            "INSERT INTO beers(name, price) VALUES (?,?)", beer.beerName, beer.beerPrice
        )
    }

    override fun getBeers(): String {
        var result : MutableList<Beer?> = jdbcTemplate.query(
            "SELECT * FROM beers", BeerRowMapper()
        )
    ...
    override fun updateBeer(beer: Beer) {
        ...
        jdbcTemplate.update(
            "UPDATE beers SET name = ?, price = ? WHERE id = ?", beer.beerName, beer.beerPrice, beer.beerID
        )
    }
    ...
    override fun deleteBeer(name: String) {
        ...
        jdbcTemplate.update(
            "DELETE FROM beers WHERE name = ?", name
        )
    }
