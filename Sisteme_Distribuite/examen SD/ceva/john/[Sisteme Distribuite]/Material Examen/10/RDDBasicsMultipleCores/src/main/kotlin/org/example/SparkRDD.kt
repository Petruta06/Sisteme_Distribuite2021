...
fun main(args: Array<String>) {
    ...
    val sparkConf = SparkConf()
        ...
        .set("spark.cores.max","spark.deploy.defaultCores.value")
        .set("spark.yarn.maxAppAttempts", "6")

    // initializarea contextului Spark
    val sparkContext = JavaSparkContext(sparkConf)
    sparkContext.setLogLevel("ERROR")

    val items : List<String> = listOf("123/643/7563/2134/ALPHA",
                                      "2343/6356/BETA/2342/12",
                                      "23423/656/343")
    ...
    // pentru utilizarea unui RDD de mai multe ori, trebuie aplicata metoda persist
    lines.persist(StorageLevel.MEMORY_ONLY())

    // functia de mapare reprezinta o transformare a setului de date initial (nu este calculat imediat)
    // abia cand se ajunge la functia de reducere (o actiune), Spark imparte operatiile in task-uri
    ...
    sparkContext.parallelize(listOf(1, 2, 3, 4)).foreach {
        x -> accumulator.add(x.toLong())
    }
    println(accumulator)
    sparkContext.stop()
}
