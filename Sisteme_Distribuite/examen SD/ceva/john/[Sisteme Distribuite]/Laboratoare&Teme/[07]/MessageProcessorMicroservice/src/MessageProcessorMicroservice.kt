        ...
        val receiveInQueueSubscription = receiveInQueueObservable
            .subscribeBy(
                onNext = {
                    ...
                    println(message)
                    messageQueue = LinkedList<Message>(messageQueue.filter {messageIn ->  !messageIn.body.toLowerCase().equals(message.body.toLowerCase())})
                    messageQueue.add(message)
                },
                onComplete = {
                    // s-a incheiat primirea tuturor mesajelor
                    messageQueue = LinkedList<Message>(messageQueue.sortedBy {it.timestamp.time })
