...
class AuctioneerMicroservice {
    ...
    private lateinit var supraveghetorSocket:Socket
    ...
    init {
        ...
        receiveBidsObservable = Observable.create<String> { emitter ->
            ...
            while (true) {
                ...
                //trimit mesaje catre supraveghetor
                try {
                    supraveghetorSocket = Socket(MESSAGE_PROCESSOR_HOST, SUPRAVEGHETOR_PORT)
                    ...
                    Observable.fromIterable(bidQueue).subscribeBy(
                        onNext = {
                            ...
                            // trimit mesajele pe care le iau de la bidderi si catre supraveghetor
                            supraveghetorSocket.getOutputStream().write(it.serialize())
                        },
                        onComplete = {
                            val noMoreMessages = Message.create(
                                "${supraveghetorSocket.localAddress}:${supraveghetorSocket.localPort}",
                                "final"
                            )
                            supraveghetorSocket.getOutputStream().write(noMoreMessages.serialize())
                            supraveghetorSocket.close()
                    ...
                    println("Nu ma pot conecta la Supraveghetor!")
    ...
    private fun finishAuction() {
        ...
        try {
            ...
            val winningPrice = result.body.split(" ")[6].toInt()
